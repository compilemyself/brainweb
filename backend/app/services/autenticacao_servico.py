...
class AutenticacaoServico:
    def __init__(self, repo: UsuarioRepo): self._repo = repo
    ...
    def registrar(self, dados: UsuarioCreate) -> Usuario:
        ...
        return self._repo.criar(Usuario(nome=dados.nome, email=dados.email, senha_hash=hash_senha(dados.senha)))
    
    def login(self, email: str, senha: str) -> TokenSchema:
        usuario = self._repo.procurar_por_email(email)
        if not usuario or not verificar_senha(senha, usuario.senha_hash): raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="E-mail ou senha incorretos.")
    ...
    def atualizar_preferencias(self, usuario, cor_mapa: str):
        usuario.cor_mapa = cor_mapa.lower(); return self._repo.atualizar(usuario)